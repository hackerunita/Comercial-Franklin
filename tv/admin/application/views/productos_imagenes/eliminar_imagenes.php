    ...
    <div id="center-column">
        ...
        <div class="container_12">
            <div class="grid_8">
                ...
                <?php
                if (isset($Mensaje_Imagen_Eliminada)) {
                    echo '<div class="grid_7 alpha">';
                    if (!empty($Mensaje_Imagen_Eliminada)) {
                        echo '<div id="formato_de_exito">';
                        echo 'Imagen Eliminada';
                        echo '</div>';
                    } else {
                        echo '<div id="formato_de_error">';
                        echo 'No se pudo eliminar la imagen, intentelo nuevamente!';
                        echo '</div>';
                    }
                    echo '</div>';
                    unset($Mensaje_Imagen_Eliminada);
                }
                ?>
                ...
                if (isset($obtenerImagenesProductoSeleccionado)) {
                    if (!empty($obtenerImagenesProductoSeleccionado)) {
                        ...
                        foreach ($obtenerImagenesProductoSeleccionado as $filaObtenerImagenesProductoSeleccionado):
                            ...
                            $nombre_imagen = $d[2];
                            echo '<li><a href="' . base_url() . '/productos_imagenes/eliminarImagen/'. $filaObtenerImagenesProductoSeleccionado->idproductos_imagenes.'/'.$filaObtenerImagenesProductoSeleccionado->productos_idproductos.'"><img src="' . base_url() . '/' . $carpeta_principal . '/' . $nombre_imagen . '" width="75" height="75" /></a></li>';
                        endforeach;
                        echo '</ul>';
                        echo '</div>';
                        echo '</div>';
